...
import {
  ...
  Sparkles,Cpu as CpuIcon,Zap,Circle,PauseCircle,PlayCircle,Share
} from 'lucide-react';
...
function Brand({compact=false}){return <div className={'brand '+(compact?'compact':'')}><Mark/>{!compact&&<div><strong>Touch<span>Workstation</span></strong><small>Ubuntu Mobile</small></div>}</div>}
...
function SetupWizard({me,onDone}){
 ...
 {step===0&&<SetupPane kicker="WELCOME" title={`Make ${me.hostname} feel native on your phone.`} body="TouchWorkstation turns this Ubuntu computer into a touch-first mobile workspace without replacing your desktop."/>}
 ...
 {step===4&&<div><SetupPane kicker="READY" title="Your mobile workspace is ready." body="Open the same address on your iPhone or iPad and add it to your Home Screen."/><div className="ready-grid"><Ready text="Ubuntu Mobile shell"/><Ready text="Projects & Git"/><Ready text="Applications"/><Ready text="Files & terminal"/></div></div>}
...
function Shell({me,settings,setSettings}){
 ...
 {!mobile&&<aside className="sidebar"><Brand/><nav>{NAV.map(([id,Icon,label])=><button key={id} className={view===id&&!project?'active':''} onClick={()=>go(id)}><Icon/><span>{label}</span></button>)}</nav><div className="sidebar-bottom"><div className="machine-chip"><Dot/><div><strong>{me.hostname}</strong><small>Connected</small></div></div><span className="version">{me.version}</span></div></aside>}
 <main className="main"><Topbar me={me} mobile={mobile} view={view} project={project} onMenu={()=>setMobileMenu(!mobileMenu)} go={go}/><div className={'view'+(transitioning?' view-transition':'')}><Router view={view} go={go} openProject={openProject} openWebview={openWebview} openAgent={openAgent} navState={navState} me={me} project={project} setProject={setProject} settings={settings} setSettings={setSettings}/></div></main>
 {mobile&&<MobileDock view={view} project={project} go={go}/>} {mobile&&mobileMenu&&<MobileSheet go={go} onClose={()=>setMobileMenu(false)}/>}</div>
}
...
function Router(p){
 ...
 switch(p.view){
   ...
   default:return <Dashboard me={p.me} go={p.go} openProject={p.openProject} settings={p.settings}/>;
 }
}
...
function Dashboard({me,go,openProject,settings}){
 ...
 return <div className="dashboard page-pad">
  <section className="mobile-hero"><div className="hero-content"><span className="kicker">{greeting()} · UBUNTU MOBILE</span><h1>{me.hostname}</h1><div className="hero-status"><span className="live-dot"><i/></span> Connected{status?<> · <b>{status.cpu}%</b> CPU · <b>{status.memory}%</b> MEM</>:''}</div></div><div className="wave"><i/><i/><i/></div></section>
