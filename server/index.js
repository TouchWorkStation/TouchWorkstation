...
app.get('/api/status',auth,async(req,res)=>{
  ...
  res.json({
    cpu,memory,disk,ip,uptime:os.uptime(),
    specs:{
...
app.post('/api/github/login',auth,async(req,res)=>{const ok=sync('command -v gh');if(!ok)return res.json({message:'GitHub CLI is not installed. Install `gh` from Ubuntu packages, then run `gh auth login` in Terminal.'});res.json({message:'Open Terminal and run: gh auth login. TouchWorkstation will use the credentials stored by GitHub CLI on this workstation.'})});
